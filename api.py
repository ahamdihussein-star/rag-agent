...
# Fallback character-based splitter (for very short texts)
...
# FastAPI app
app = FastAPI(title="RAG Agent API", version="2.2.0")
...
def smart_chunk_text(text: str, min_chunk_size: int = 100) -> List[str]:
    ...
    2. Fall back to character-based if needed
    3. Merge very small chunks
    """
    ...
    try:
        ...
        for chunk in chunks:
            if len(current_chunk) + len(chunk) < 1500:  # Max chunk size
        ...
        # If semantic chunking produced too few chunks, use hybrid approach
...
def chunk_by_sections(text: str) -> List[str]:
    """
    Alternative: Chunk by detecting sections/headers
...
def save_full_document(doc_id: str, content: str, metadata: dict):
    """Save full document to local storage"""
...
def load_full_document(doc_id: str) -> Optional[dict]:
    """Load full document from local storage"""
    doc_path = os.path.join(DOCUMENTS_FOLDER, f"{doc_id}.json")
...
def detect_source_filter(query: str) -> Optional[dict]:
    """Use LLM to detect what type of source the user is asking about"""
    prompt = f"""Analyze this query and determine the source type:

Query: {query}

Options:
- "document" = job offers, contracts, resumes, uploaded PDFs/docs
- "website" = website content, company info from web
- "youtube" = video content
- "all" = general or could apply to any

Respond with ONE word only: document, website, youtube, or all"""
    ...
    try:
        ...
        if source_type == "document":
            ...
            return {"type": {"$eq": "website"}}
        elif source_type == "youtube":
            return {"type": {"$eq": "youtube"}}
        else:
            return {"type": {"$ne": "memory"}}
    except:
        return {"type": {"$ne": "memory"}}
...
def hybrid_search(query: str, top_k: int = 10, source_filter: dict = None) -> List[dict]:
    ...
    filter_dict = source_filter if source_filter else {"type": {"$ne": "memory"}}
...
def get_document_context_with_sources(query: str, top_k: int = 10, display_min_score: float = 0.30):
    """Hybrid Parent-Child Retrieval with Semantic Chunks"""
    
    source_filter = detect_source_filter(query)
    search_results = hybrid_search(query, top_k=top_k, source_filter=source_filter)
...
@app.get("/")
def root():
    return {"message": "🤖 RAG Agent API v2.2 - Semantic Chunking + Hybrid Search"}
...
def chat(request: ChatRequest):
    """Chat with Semantic Chunking + Hybrid Search"""
...
Available Information (Full Documents):
{doc_context if doc_context.strip() else "No specific documents found."}
...
def process_youtube(request: UrlRequest):
    """Process YouTube with Semantic Chunking"""
    try:
        url = request.url
...
def get_document_detail(doc_id: str):
    ...
    chunks = []
    try:
        # Query Pinecone for chunks with this parent_id
...
def delete_document(doc_id: str):
    ...
    try:
        # Find and delete all chunks with this parent_id
        dummy_vector = embeddings.embed_query("test")
        results = index.query(
            vector=dummy_vector,
            top_k=100,
...
def update_document_metadata(doc_id: str, title: str = None, summary: str = None):
